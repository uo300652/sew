...
def main():
    ...
    videos_xml = raiz.findall("u:videos/u:video", ns)
    if videos_xml:
        html.add_parrafo("Vídeos:")
        for v in videos_xml:
            url = v.attrib.get("url", "")
            if not url:
                continue

            descripcion = v.attrib.get("descripcion", "").strip()
            if descripcion:
                # Texto descriptivo antes del vídeo (ayuda a la accesibilidad)
                html.add_parrafo(descripcion)

            # <video src="..." controls preload="auto">Texto fallback</video>
            video_elem = ET.SubElement(html.main, "video", attrib={
                "src": url,
                "controls": "controls",
                "preload": "auto"
            })
            # Texto dentro del vídeo como contenido alternativo
            video_elem.text = "Tu navegador no soporta el elemento vídeo."
